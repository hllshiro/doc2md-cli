# 核心架构设计

<cite>
**本文引用的文件**
- [src/main.ts](file://src/main.ts)
- [src/context.ts](file://src/context.ts)
- [src/runner.ts](file://src/runner.ts)
- [src/logger.ts](file://src/logger.ts)
- [src/tasks/docxInput.ts](file://src/tasks/docxInput.ts)
- [src/tasks/pandocCheck.ts](file://src/tasks/pandocCheck.ts)
- [src/tasks/docxConvert.ts](file://src/tasks/docxConvert.ts)
- [src/tasks/mediaConvert.ts](file://src/tasks/mediaConvert.ts)
- [src/tasks/mdCleanup.ts](file://src/tasks/mdCleanup.ts)
- [src/tasks/imageRecognition/index.ts](file://src/tasks/imageRecognition/index.ts)
- [src/tasks/imageRecognition/configureTask.ts](file://src/tasks/imageRecognition/configureTask.ts)
- [src/tasks/imageRecognition/processTask.ts](file://src/tasks/imageRecognition/processTask.ts)
- [src/tasks/imageRecognition/recognition.ts](file://src/tasks/imageRecognition/recognition.ts)
- [src/tasks/imageRecognition/constants.ts](file://src/tasks/imageRecognition/constants.ts)
- [src/tasks/imageRecognition/helpers.ts](file://src/tasks/imageRecognition/helpers.ts)
- [src/tasks/imageRecognition/types.ts](file://src/tasks/imageRecognition/types.ts)
- [src/utils.ts](file://src/utils.ts)
- [scripts/build.mjs](file://scripts/build.mjs)
- [scripts/release.mjs](file://scripts/release.mjs)
- [sea-config.json](file://sea-config.json)
- [package.json](file://package.json)
- [tsconfig.json](file://tsconfig.json)
- [eslint.config.js](file://eslint.config.js)
</cite>

## 更新摘要
**所做更改**
- 更新项目名称从 doc2xml-cli 到 doc2md-cli
- 新增 AI 图片识别与替换任务模块
- 增强日志记录系统，支持详细执行日志
- 添加超时控制和重试机制
- 扩展上下文状态管理，支持恢复点功能
- 增强任务编排器的跳过功能

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件面向高级开发者，系统性阐述 Doc2MD CLI 的核心架构设计。该应用以"任务驱动 + 列表器编排"的方式组织多阶段转换流程：从用户输入、环境检测、文档转换、矢量图渲染与路径修正，再到最终的 Markdown 清理与输出。整体采用基于 Listr2 的任务编排系统，结合上下文状态管理（AppContext/OutputContext）实现模块化、可扩展的任务流水线；同时通过 SEA 打包技术将 Node.js 可执行文件与资源一体化分发，提升部署一致性与运行时稳定性。

**更新** 项目已重命名为 doc2md-cli，新增 AI 图片识别与替换功能，增强了实时进度跟踪、超时控制和配置管理等新特性。

## 项目结构
项目采用"功能域 + 层次化"组织方式：
- 根入口与编排层：main.ts、runner.ts、context.ts、logger.ts
- 任务层：tasks/*.ts（每个任务封装为 Listr 任务对象）
- AI 图片识别模块：tasks/imageRecognition/*（配置、识别、处理任务）
- 工具与缓存：utils.ts（输入缓存、提示样式等）
- 构建与发布：scripts/build.mjs、scripts/release.mjs
- 配置：sea-config.json、package.json、tsconfig.json、eslint.config.js

```mermaid
graph TB
subgraph "应用入口与编排"
MAIN["src/main.ts"]
RUNNER["src/runner.ts"]
CTX["src/context.ts"]
LOG["src/logger.ts"]
end
subgraph "任务层"
T1["src/tasks/docxInput.ts"]
T2["src/tasks/pandocCheck.ts"]
T3["src/tasks/docxConvert.ts"]
T4["src/tasks/mediaConvert.ts"]
T5["src/tasks/mdCleanup.ts"]
T6["src/tasks/imageRecognition/index.ts"]
end
subgraph "AI 图片识别模块"
AI1["configureTask.ts"]
AI2["processTask.ts"]
AI3["recognition.ts"]
AI4["constants.ts"]
AI5["helpers.ts"]
AI6["types.ts"]
end
subgraph "工具与配置"
UTIL["src/utils.ts"]
PKG["package.json"]
SEA["sea-config.json"]
TS["tsconfig.json"]
ESL["eslint.config.js"]
end
subgraph "构建与发布"
BUILD["scripts/build.mjs"]
REL["scripts/release.mjs"]
end
MAIN --> RUNNER
MAIN --> CTX
MAIN --> LOG
RUNNER --> T1 --> T2 --> T3 --> T4 --> T5 --> T6
T6 --> AI1 --> AI2
AI2 --> AI3
AI2 --> AI4
AI2 --> AI5
AI2 --> AI6
T1 -. 使用 .-> UTIL
T3 -. 使用 .-> CTX
T4 -. 使用 .-> CTX
T5 -. 使用 .-> CTX
T6 -. 使用 .-> CTX
LOG -. 记录 .-> MAIN
LOG -. 记录 .-> RUNNER
BUILD --> SEA
BUILD --> PKG
REL --> PKG
```

**图表来源**
- [src/main.ts:1-11](file://src/main.ts#L1-L11)
- [src/runner.ts:1-10](file://src/runner.ts#L1-L10)
- [src/context.ts:1-81](file://src/context.ts#L1-L81)
- [src/logger.ts:1-104](file://src/logger.ts#L1-L104)
- [src/tasks/docxInput.ts:1-52](file://src/tasks/docxInput.ts#L1-L52)
- [src/tasks/pandocCheck.ts:1-24](file://src/tasks/pandocCheck.ts#L1-L24)
- [src/tasks/docxConvert.ts:1-64](file://src/tasks/docxConvert.ts#L1-L64)
- [src/tasks/mediaConvert.ts:1-112](file://src/tasks/mediaConvert.ts#L1-L112)
- [src/tasks/mdCleanup.ts:1-373](file://src/tasks/mdCleanup.ts#L1-L373)
- [src/tasks/imageRecognition/index.ts:1-11](file://src/tasks/imageRecognition/index.ts#L1-L11)
- [src/tasks/imageRecognition/configureTask.ts:1-126](file://src/tasks/imageRecognition/configureTask.ts#L1-L126)
- [src/tasks/imageRecognition/processTask.ts:1-301](file://src/tasks/imageRecognition/processTask.ts#L1-L301)
- [src/tasks/imageRecognition/recognition.ts:1-269](file://src/tasks/imageRecognition/recognition.ts#L1-L269)
- [src/tasks/imageRecognition/constants.ts:1-66](file://src/tasks/imageRecognition/constants.ts#L1-L66)
- [src/tasks/imageRecognition/helpers.ts:1-119](file://src/tasks/imageRecognition/helpers.ts#L1-L119)
- [src/tasks/imageRecognition/types.ts:1-31](file://src/tasks/imageRecognition/types.ts#L1-L31)
- [src/utils.ts:1-50](file://src/utils.ts#L1-L50)
- [scripts/build.mjs:1-53](file://scripts/build.mjs#L1-L53)
- [scripts/release.mjs:1-42](file://scripts/release.mjs#L1-L42)
- [sea-config.json:1-6](file://sea-config.json#L1-L6)
- [package.json:1-42](file://package.json#L1-L42)
- [tsconfig.json:1-19](file://tsconfig.json#L1-L19)
- [eslint.config.js:1-26](file://eslint.config.js#L1-L26)

**章节来源**
- [src/main.ts:1-73](file://src/main.ts#L1-L73)
- [src/runner.ts:1-10](file://src/runner.ts#L1-L10)
- [src/context.ts:1-81](file://src/context.ts#L1-L81)
- [src/logger.ts:1-104](file://src/logger.ts#L1-L104)
- [package.json:1-42](file://package.json#L1-L42)

## 核心组件
- 应用入口与控制流：main.ts 负责创建上下文、创建 Runner 并注册任务，随后统一调度执行；新增跳过功能支持从指定任务开始执行；异常处理对用户中断与错误进行区分处理。
- 任务编排器：runner.ts 基于 Listr2 创建 Runner 实例，配置渲染选项与上下文注入。
- 上下文模型：context.ts 定义 AppContext（输入路径、输出路径、pandoc 可执行路径、恢复点）与 OutputContext（单轮输出产物的文件名、路径、媒体目录），用于任务间传递状态。
- 日志系统：logger.ts 提供进程级日志记录，支持 DEBUG/INFO/WARN/ERROR 级别，自动创建带时间戳的日志文件。
- 任务集合：tasks/*.ts 将各阶段封装为 Listr 任务对象，具备标题、任务体与子任务（如媒体转换中的两步子任务）。
- AI 图片识别模块：imageRecognition/* 提供完整的 AI 图像识别与替换功能，包括配置、识别、处理和工具函数。
- 工具与缓存：utils.ts 提供输入缓存（持久化到用户主目录）与提示样式辅助函数，改善用户体验。
- 构建与发布：scripts/build.mjs 生成 CJS bundle、SEA blob、注入 blob、复制 .NET 模块；scripts/release.mjs 组织发行包。
- 配置：sea-config.json 指定 SEA 主入口与输出；package.json 定义依赖与脚本；tsconfig.json/ eslint.config.js 规范工程化。

**更新** 新增 AI 图片识别模块，增强日志记录系统，扩展上下文状态管理以支持恢复点功能。

**章节来源**
- [src/main.ts:1-73](file://src/main.ts#L1-L73)
- [src/runner.ts:1-10](file://src/runner.ts#L1-L10)
- [src/context.ts:1-81](file://src/context.ts#L1-L81)
- [src/logger.ts:1-104](file://src/logger.ts#L1-L104)
- [src/tasks/imageRecognition/index.ts:1-11](file://src/tasks/imageRecognition/index.ts#L1-L11)
- [src/tasks/imageRecognition/configureTask.ts:1-126](file://src/tasks/imageRecognition/configureTask.ts#L1-L126)
- [src/tasks/imageRecognition/processTask.ts:1-301](file://src/tasks/imageRecognition/processTask.ts#L1-L301)
- [src/tasks/imageRecognition/recognition.ts:1-269](file://src/tasks/imageRecognition/recognition.ts#L1-L269)
- [src/utils.ts:1-50](file://src/utils.ts#L1-L50)
- [scripts/build.mjs:1-53](file://scripts/build.mjs#L1-L53)
- [scripts/release.mjs:1-42](file://scripts/release.mjs#L1-L42)
- [sea-config.json:1-6](file://sea-config.json#L1-L6)
- [package.json:1-42](file://package.json#L1-L42)
- [tsconfig.json:1-19](file://tsconfig.json#L1-L19)
- [eslint.config.js:1-26](file://eslint.config.js#L1-L26)

## 架构总览
Doc2MD CLI 的架构围绕"任务驱动 + 上下文状态 + SEA 打包"展开：
- 任务驱动：每个阶段（输入、环境检测、转换、矢量图处理、清理、AI 图片识别）独立封装为 Listr 任务，便于测试、复用与扩展。
- 上下文状态：AppContext 作为全局上下文，OutputContext 作为每轮输出的局部上下文，任务通过修改上下文推进流程。
- 编排系统：Listr2 提供可视化进度、子任务嵌套、并发控制与错误传播。
- SEA 打包：将 Node.js 运行时与应用资源一体化，生成自包含可执行文件，降低部署复杂度。
- 日志系统：提供详细的执行日志记录，支持调试和问题排查。

**更新** 新增 AI 图片识别任务链，增强实时进度跟踪和超时控制机制。

```mermaid
sequenceDiagram
participant U as "用户"
participant M as "main.ts"
participant R as "Runner(Listr)"
participant T1 as "docxInput"
participant T2 as "pandocCheck"
participant T3 as "docxConvert"
participant T4 as "mediaConvert"
participant T5 as "mdCleanup"
participant T6 as "imageRecognition"
U->>M : 启动应用
M->>R : 创建 Runner 并注入上下文
M->>R : 注册任务序列
R->>T1 : 执行输入任务
T1-->>R : 写入 inputPath/outputPath
R->>T2 : 执行环境检测
T2-->>R : 写入 pandocExec
R->>T3 : 执行转换
T3-->>R : 写入 lastContext(输出文件/媒体)
R->>T4 : 执行媒体转换与路径修正
T4-->>R : 更新 lastContext
R->>T5 : 执行 Markdown 清理
T5-->>R : 更新 lastContext
R->>T6 : 执行 AI 图片识别
T6-->>R : 更新最终结果
R-->>U : 显示结果与进度
```

**图表来源**
- [src/main.ts:23-31](file://src/main.ts#L23-L31)
- [src/tasks/docxInput.ts:27-51](file://src/tasks/docxInput.ts#L27-L51)
- [src/tasks/pandocCheck.ts:14-23](file://src/tasks/pandocCheck.ts#L14-L23)
- [src/tasks/docxConvert.ts:10-63](file://src/tasks/docxConvert.ts#L10-L63)
- [src/tasks/mediaConvert.ts:104-111](file://src/tasks/mediaConvert.ts#L104-L111)
- [src/tasks/mdCleanup.ts:331-372](file://src/tasks/mdCleanup.ts#L331-L372)
- [src/tasks/imageRecognition/index.ts:6-10](file://src/tasks/imageRecognition/index.ts#L6-L10)

## 详细组件分析

### 任务编排器与上下文管理
- Runner 创建：通过 createRunner(ctx) 基于 Listr2 初始化，设置渲染选项与上下文注入，保证任务间共享状态。
- 上下文模型：
  - AppContext：输入路径、输出根目录、pandoc 可执行路径；lastContext 作为 OutputContext 的可选字段，承载最近一轮输出产物信息；新增 startFrom 字段支持从指定任务开始执行。
  - OutputContext：单轮输出的文件名、输出路径、媒体目录，便于后续任务复用。
- 设计要点：使用不可变的上下文对象，任务只读取/写入必要字段，避免耦合；通过 lastContext 实现"链式产出"，减少重复计算。
- 恢复点功能：RESUME_POINTS 定义了多个恢复点，支持从不同阶段重新开始执行。

**更新** 新增 startFrom 字段和恢复点功能，支持任务跳过和断点续执行。

```mermaid
classDiagram
class AppContext {
+string inputPath
+string outputPath
+string pandocExec
+OutputContext lastContext?
+number startFrom?
}
class OutputContext {
+string outFilename
+string outputPath
+string mediaPath
}
class Runner {
+add(task)
+run()
}
class ProcessLogger {
+getLogPath()
+debug(msg, task?)
+info(msg, task?)
+warn(msg, task?)
+error(msg, task?)
}
Runner --> AppContext : "持有"
AppContext --> OutputContext : "可选引用"
ProcessLogger --> Runner : "记录日志"
```

**图表来源**
- [src/context.ts:10-25](file://src/context.ts#L10-L25)
- [src/context.ts:27-32](file://src/context.ts#L27-L32)
- [src/runner.ts:4-9](file://src/runner.ts#L4-L9)
- [src/logger.ts:8-104](file://src/logger.ts#L8-L104)

**章节来源**
- [src/runner.ts:1-10](file://src/runner.ts#L1-L10)
- [src/context.ts:1-81](file://src/context.ts#L1-L81)
- [src/logger.ts:1-104](file://src/logger.ts#L1-L104)

### 输入任务（docxInput）
- 功能：交互式收集 .docx 路径，校验存在性，支持缓存默认值；根据绝对/相对路径设置 inputPath 与输出目录。
- 交互：使用 Inquirer 与 Listr Inquirer 适配器，提供带颜色提示与默认值。
- 缓存：通过 utils.loadCache/saveCache 将最近一次输入持久化至用户主目录，提升可用性。
- 错误处理：路径为空或不存在时返回明确错误信息，交由 Runner 统一处理。

```mermaid
flowchart TD
Start(["开始"]) --> LoadCache["加载缓存"]
LoadCache --> Prompt["交互提示输入路径"]
Prompt --> Validate{"路径有效?"}
Validate --> |否| Error["返回错误信息"]
Validate --> |是| SaveCache["保存缓存"]
SaveCache --> AbsCheck{"是否绝对路径?"}
AbsCheck --> |是| SetAbs["设置 inputPath 与输出目录"]
AbsCheck --> |否| SetRel["拼接当前工作目录并设置输出目录"]
SetAbs --> End(["结束"])
SetRel --> End
Error --> End
```

**图表来源**
- [src/tasks/docxInput.ts:27-51](file://src/tasks/docxInput.ts#L27-L51)
- [src/utils.ts:28-49](file://src/utils.ts#L28-L49)

**章节来源**
- [src/tasks/docxInput.ts:1-52](file://src/tasks/docxInput.ts#L1-L52)
- [src/utils.ts:1-50](file://src/utils.ts#L1-L50)

### 环境检测任务（pandocCheck）
- 功能：检测系统是否已安装 pandoc；若未安装则抛出错误，阻止后续流程。
- 设计：纯函数 testGlobalInstall，职责单一，便于测试与替换策略。

```mermaid
flowchart TD
S(["开始"]) --> Exec["执行 pandoc --version"]
Exec --> Ok{"返回成功?"}
Ok --> |是| SetPandoc["设置 pandocExec='pandoc'"]
Ok --> |否| ThrowErr["抛出未安装错误"]
SetPandoc --> E(["结束"])
ThrowErr --> E
```

**图表来源**
- [src/tasks/pandocCheck.ts:5-23](file://src/tasks/pandocCheck.ts#L5-L23)

**章节来源**
- [src/tasks/pandocCheck.ts:1-24](file://src/tasks/pandocCheck.ts#L1-L24)

### 文档转换任务（docxConvert）
- 功能：调用 pandoc 将 .docx 转换为 Markdown，并提取媒体资源；成功后写入 lastContext。
- 关键点：使用子进程调用 pandoc，监听 stderr 收集错误；在 close 事件中根据退出码决定成功/失败；创建输出目录与媒体目录。
- 输出：设置 lastContext.outFilename、outputPath、mediaPath，供后续任务使用。

```mermaid
sequenceDiagram
participant R as "Runner"
participant T as "docxConvert"
participant P as "pandoc(子进程)"
R->>T : 执行任务
T->>T : 创建输出目录
T->>P : spawn pandoc args
P-->>T : stderr 数据累积
P-->>T : close(code)
alt code == 0
T->>R : 写入 lastContext
T-->>R : resolve()
else code != 0
T-->>R : reject(stderr)
end
```

**图表来源**
- [src/tasks/docxConvert.ts:10-63](file://src/tasks/docxConvert.ts#L10-L63)

**章节来源**
- [src/tasks/docxConvert.ts:1-64](file://src/tasks/docxConvert.ts#L1-L64)

### 媒体转换任务（mediaConvert）
- 功能：将 EMF/WMF 矢量图渲染为 JPG，并更新 Markdown 中的图片引用路径；同时复制 MD 文件到新目录。
- 设计模式：采用"子任务列表 + 新建子编排"的方式，先渲染图片，再修补 Markdown，顺序严格且互不并发。
- 资源定位：根据运行时（SEA）与开发环境自动定位 .NET 转换器可执行文件，兼容不同部署形态。
- 状态更新：更新 lastContext.mediaPath 与 outputPath，确保后续清理任务使用最新路径。

```mermaid
flowchart TD
S(["开始"]) --> Detect["扫描 media 目录中的 EMF/WMF"]
Detect --> HasFiles{"是否有目标文件?"}
HasFiles --> |否| Skip["输出跳过信息"] --> End(["结束"])
HasFiles --> |是| Render["逐个调用转换器渲染为 JPG"]
Render --> Patch["读取 MD，替换图片引用为 JPG"]
Patch --> Write["写出新 MD 到新目录"]
Write --> Update["更新 lastContext.mediaPath/outputPath"]
Update --> End
```

**图表来源**
- [src/tasks/mediaConvert.ts:104-111](file://src/tasks/mediaConvert.ts#L104-L111)
- [src/tasks/mediaConvert.ts:43-72](file://src/tasks/mediaConvert.ts#L43-L72)
- [src/tasks/mediaConvert.ts:75-102](file://src/tasks/mediaConvert.ts#L75-L102)

**章节来源**
- [src/tasks/mediaConvert.ts:1-112](file://src/tasks/mediaConvert.ts#L1-L112)

### Markdown 清理任务（mdCleanup）
- 功能：移除 pandoc 生成 Markdown 中的 HTML 片段与冗余标记，修复标题层级与图片引用，输出干净的 Markdown。
- 设计模式：采用"有限状态机 + 正则匹配 + 分块处理"的纯函数清洗逻辑，便于单元测试与演进。
- 状态机：NORMAL/IN_ZHENGWEN/IN_HEADING/IN_FIGURE/IN_TABLE/IN_IMG 多状态切换，处理跨行与嵌套结构。
- 输出：创建输出目录，写出清理后的 Markdown，并更新 lastContext，保留媒体目录不变。

```mermaid
flowchart TD
S(["开始"]) --> Split["按行拆分源文本"]
Split --> FSM["状态机逐行处理"]
FSM --> Rules{"命中规则?"}
Rules --> |正文段落| EnterZW["进入正文块"]
Rules --> |标题块| EnterHD["解析中文序号→ATX前缀"]
Rules --> |图片块| EnterIMG["累积多行 <img>"]
Rules --> |表格块| EnterTB["累积表格内容"]
EnterZW --> EmitZW["丢弃标签，保留正文"]
EnterHD --> EmitHD["输出 ATX 标题"]
EnterIMG --> EmitIMG["转为 Markdown 图片语法"]
EnterTB --> EmitTB["原样输出表格"]
EmitZW --> Next["继续下一行"]
EmitHD --> Next
EmitIMG --> Next
EmitTB --> Next
Next --> FSM
FSM --> EOF{"到达文件末尾?"}
EOF --> |是| Finalize["收尾处理未闭合块"]
Finalize --> Write["写出清理后 Markdown"]
Write --> Update["更新 lastContext"]
Update --> End(["结束"])
```

**图表来源**
- [src/tasks/mdCleanup.ts:331-372](file://src/tasks/mdCleanup.ts#L331-L372)
- [src/tasks/mdCleanup.ts:7-327](file://src/tasks/mdCleanup.ts#L7-L327)

**章节来源**
- [src/tasks/mdCleanup.ts:1-373](file://src/tasks/mdCleanup.ts#L1-L373)

### AI 图片识别任务链（imageRecognition）

#### 配置任务（configureTask）
- 功能：交互式配置 AI 视觉识别接口参数，包括基础 URL、API Key、模型选择、校验开关和超时设置。
- 交互：使用 Inquirer 与 Listr Inquirer 适配器，提供带颜色提示与默认值。
- 模型发现：自动从 AI 接口获取可用模型列表，支持动态选择。
- 缓存：配置参数会持久化到用户缓存，提升后续使用体验。

#### 处理任务（processTask）
- 功能：扫描 Markdown 文件中的图片引用，调用 AI 进行内容识别与替换。
- 实时进度：使用定时器显示每张图片的识别进度和耗时，支持超时显示。
- 重试机制：支持多次重试失败的图片，提供用户确认界面。
- 替换策略：根据识别结果类型（ASCII、LaTeX、描述）采用不同的 Markdown 替换策略。
- 状态管理：自动保存中间状态到 context.json，支持断点续执行。

#### 识别引擎（recognition）
- 超时控制：提供 withTimeout 函数，支持可配置的超时时间和 AbortController 取消机制。
- 结果解析：智能解析 AI 返回的 JSON 结果，处理各种转义字符和格式问题。
- 校验机制：可选的结果校验功能，通过 VALIDATION_PROMPT 对识别结果进行二次验证。
- 重试逻辑：支持最多 MAX_RECOGNITION_ATTEMPTS 次重试，每次重试都会基于反馈改进提示词。

#### 工具函数（helpers）
- 路径解析：智能解析图片路径，优先使用相对路径，回退到媒体目录。
- 正则匹配：使用 RE_MD_IMAGE 正则表达式精确匹配 Markdown 图片语法。
- MIME 类型：根据文件扩展名确定正确的 MIME 类型。
- 替换构建：根据识别结果和图片上下文构建合适的 Markdown 替换内容。

```mermaid
flowchart TD
AIStart(["AI 图片识别开始"]) --> AIConfig["配置任务：获取接口参数"]
AIConfig --> AIModels["获取模型列表"]
AIModels --> AIProcess["处理任务：扫描图片引用"]
AIProcess --> AIMatch["正则匹配图片语法"]
AIMatch --> AIRead["读取图片文件"]
AIRead --> AIRecognize["调用 AI 识别"]
AIRecognize --> AIResult{"识别成功?"}
AIResult --> |是| AIReplace["构建替换内容"]
AIResult --> |否| AIRetry["重试或跳过"]
AIRetry --> AIRead
AIReplace --> AIWrite["写出替换结果"]
AIWrite --> AIEnd(["AI 图片识别完成"])
```

**图表来源**
- [src/tasks/imageRecognition/index.ts:6-10](file://src/tasks/imageRecognition/index.ts#L6-L10)
- [src/tasks/imageRecognition/configureTask.ts:35-125](file://src/tasks/imageRecognition/configureTask.ts#L35-L125)
- [src/tasks/imageRecognition/processTask.ts:67-300](file://src/tasks/imageRecognition/processTask.ts#L67-L300)
- [src/tasks/imageRecognition/recognition.ts:78-268](file://src/tasks/imageRecognition/recognition.ts#L78-L268)

**章节来源**
- [src/tasks/imageRecognition/index.ts:1-11](file://src/tasks/imageRecognition/index.ts#L1-L11)
- [src/tasks/imageRecognition/configureTask.ts:1-126](file://src/tasks/imageRecognition/configureTask.ts#L1-L126)
- [src/tasks/imageRecognition/processTask.ts:1-301](file://src/tasks/imageRecognition/processTask.ts#L1-L301)
- [src/tasks/imageRecognition/recognition.ts:1-269](file://src/tasks/imageRecognition/recognition.ts#L1-L269)
- [src/tasks/imageRecognition/constants.ts:1-66](file://src/tasks/imageRecognition/constants.ts#L1-L66)
- [src/tasks/imageRecognition/helpers.ts:1-119](file://src/tasks/imageRecognition/helpers.ts#L1-L119)
- [src/tasks/imageRecognition/types.ts:1-31](file://src/tasks/imageRecognition/types.ts#L1-L31)

### 日志系统（logger）
- 单例模式：ProcessLogger 采用单例模式，确保整个进程只有一个日志实例。
- 文件管理：自动生成带时间戳的日志文件，存储在系统临时目录中。
- 多级别记录：支持 DEBUG/INFO/WARN/ERROR 四个日志级别，便于不同粒度的问题追踪。
- TTY 检测：自动检测终端类型，非 TTY 环境下禁用颜色输出。
- 自动初始化：首次使用时自动创建日志文件并写入头部信息。

**更新** 新增完整的日志系统，支持详细执行过程记录和问题排查。

```mermaid
classDiagram
class ProcessLogger {
-static instance : ProcessLogger
-logPath : string
-fileStream : WriteStream
-isTTY : boolean
+getInstance() : ProcessLogger
+reset() : void
+getLogPath() : string
+debug(msg, task?)
+info(msg, task?)
+warn(msg, task?)
+error(msg, task?)
}
```

**图表来源**
- [src/logger.ts:8-104](file://src/logger.ts#L8-L104)

**章节来源**
- [src/logger.ts:1-104](file://src/logger.ts#L1-L104)

### 构建与发布流程（SEA 打包）
- 构建步骤：
  1) 使用 esbuild 将入口打包为 dist/bundle.cjs；
  2) 生成 SEA blob（sea-config.json 指定主入口与输出）；
  3) 复制 Node 可执行文件为 dist/cli-task-tool.exe；
  4) 使用 postject 将 blob 注入可执行文件；
  5) 复制 .NET 模块到 dist/module。
- 发布步骤：校验产物完整性，拷贝到 release/<version>/<name>，压缩为 zip 包。
- SEA 优势：将 Node.js 运行时与应用资源一体化，避免运行时缺失依赖问题，简化分发与部署。

```mermaid
flowchart TD
B0["开始构建"] --> B1["esbuild 打包为 bundle.cjs"]
B1 --> B2["生成 SEA blob"]
B2 --> B3["复制 Node 可执行文件为 cli-task-tool.exe"]
B3 --> B4["postject 注入 blob"]
B4 --> B5["复制 .NET 模块到 dist/module"]
B5 --> B6["构建完成"]
R0["开始发布"] --> R1["校验 dist/artifacts"]
R1 --> R2["清理/重建 release/<ver>/<name>"]
R2 --> R3["拷贝 exe 与 module"]
R3 --> R4["压缩为 zip"]
R4 --> R5["发布完成"]
```

**图表来源**
- [scripts/build.mjs:13-53](file://scripts/build.mjs#L13-L53)
- [scripts/release.mjs:12-42](file://scripts/release.mjs#L12-L42)
- [sea-config.json:1-6](file://sea-config.json#L1-L6)

**章节来源**
- [scripts/build.mjs:1-53](file://scripts/build.mjs#L1-L53)
- [scripts/release.mjs:1-42](file://scripts/release.mjs#L1-L42)
- [sea-config.json:1-6](file://sea-config.json#L1-L6)

## 依赖分析
- 运行时依赖：listr2、@listr2/prompt-adapter-inquirer、@inquirer/prompts、@ai-sdk/openai-compatible、ai。
- 构建与打包：esbuild、postject、Node.js 原生命令（child_process、fs/promises、path）。
- 工程配置：TypeScript 编译、ESLint 规则、包管理脚本。

**更新** 新增 AI 相关依赖 @ai-sdk/openai-compatible 和 ai，用于图片识别功能。

```mermaid
graph LR
PKG["package.json"] --> LST["listr2"]
PKG --> INA["@inquirer/prompts"]
PKG --> ADP["@listr2/prompt-adapter-inquirer"]
PKG --> AIC["@ai-sdk/openai-compatible"]
PKG --> AI["ai"]
BUILD["scripts/build.mjs"] --> ESB["esbuild"]
BUILD --> PJ["postject"]
BUILD --> N["Node 原生模块"]
REL["scripts/release.mjs"] --> FS["文件系统操作"]
```

**图表来源**
- [package.json:21-40](file://package.json#L21-L40)
- [scripts/build.mjs:1-53](file://scripts/build.mjs#L1-L53)
- [scripts/release.mjs:1-42](file://scripts/release.mjs#L1-L42)

**章节来源**
- [package.json:1-42](file://package.json#L1-L42)
- [eslint.config.js:1-26](file://eslint.config.js#L1-L26)
- [tsconfig.json:1-19](file://tsconfig.json#L1-L19)

## 性能考虑
- I/O 优化：任务内部尽量批量读写与串行处理，避免不必要的并发导致磁盘争用；媒体转换阶段按需渲染，减少无效工作。
- 子进程管理：pandoc 与转换器均通过 spawn 启动，监听 stderr 与 close 事件，及时释放资源。
- 缓存策略：输入缓存减少重复交互；lastContext 避免重复计算与路径拼接。
- 构建优化：esbuild 最小化打包，禁用 sourcemap 与声明文件以缩短构建时间（生产构建建议开启）。
- AI 优化：支持超时控制和重试机制，避免长时间阻塞；智能重试基于上次反馈改进提示词。
- 日志优化：异步文件写入，避免阻塞主线程执行。

**更新** 新增 AI 性能优化策略，包括超时控制、重试机制和智能提示词改进。

## 故障排除指南
- 用户中断：捕获 ExitPromptError，优雅退出并返回特定退出码，避免等待。
- pandoc 未安装：pandocCheck 抛错，提示安装后重试。
- 转换失败：docxConvert 捕获子进程错误输出，将 stderr 作为错误消息返回。
- 资源定位失败：mediaConvert 自动在 SEA 与开发路径间切换，若仍找不到转换器，检查 dist/module 是否正确复制。
- 权限问题：确保输出目录可写，媒体目录可读写。
- 缓存异常：loadCache/saveCache 对异常进行静默处理，不影响主流程。
- AI 连接失败：configureTask 支持重试机制，检查网络连接和接口地址。
- 超时问题：recognition.ts 提供超时控制，合理设置 timeout 参数。
- 日志查看：使用 logger.getLogPath() 获取详细日志文件路径。

**更新** 新增 AI 相关故障排除指南，包括连接失败、超时问题和日志查看方法。

**章节来源**
- [src/main.ts:59-72](file://src/main.ts#L59-L72)
- [src/tasks/pandocCheck.ts:17-21](file://src/tasks/pandocCheck.ts#L17-L21)
- [src/tasks/docxConvert.ts:48-61](file://src/tasks/docxConvert.ts#L48-L61)
- [src/tasks/mediaConvert.ts:19-24](file://src/tasks/mediaConvert.ts#L19-L24)
- [src/utils.ts:40-49](file://src/utils.ts#L40-L49)
- [src/tasks/imageRecognition/configureTask.ts:63-83](file://src/tasks/imageRecognition/configureTask.ts#L63-L83)
- [src/tasks/imageRecognition/recognition.ts:18-37](file://src/tasks/imageRecognition/recognition.ts#L18-L37)
- [src/logger.ts:78-80](file://src/logger.ts#L78-L80)

## 结论
Doc2MD CLI 通过"任务驱动 + 上下文状态 + SEA 打包"的架构，实现了高内聚、低耦合的文档转换流水线。Listr2 提供了强大的可视化与错误传播能力；AppContext/OutputContext 的状态管理模式使任务间协作清晰可控；SEA 打包确保了运行时的一致性与可移植性。新增的 AI 图片识别功能进一步提升了文档转换的质量和智能化水平，实时进度跟踪和超时控制机制改善了用户体验。该设计既满足高级开发者的扩展需求，也为最终用户提供稳定可靠的转换体验。

**更新** 项目重命名为 doc2md-cli，新增 AI 图片识别模块，增强了系统的智能化和用户体验。

## 附录
- 扩展建议：
  - 新增任务：遵循现有 Listr 任务接口，仅在需要时修改上下文字段；通过 Runner.add 注册。
  - 策略模式：对于可替换的外部工具（如 pandoc 替代方案），可在 Runner 初始化时注入策略函数。
  - 工厂模式：将任务对象的创建集中到工厂函数中，便于条件注册与测试替身。
  - 并发控制：在允许的场景下使用 newListr 的并发选项，但需注意 I/O 争用与资源限制。
  - AI 扩展：可以添加更多类型的 AI 识别功能，如表格识别、公式识别等。
- 配置参考：
  - TypeScript：严格类型检查、ESM/NodeNext 模块解析。
  - ESLint：推荐规则与禁用 any 类型，保持代码质量。
  - SEA：通过 sea-config.json 指定主入口与输出，构建脚本负责生成与注入。
  - AI 配置：通过 aiConfig 对象管理 AI 接口参数，支持热更新。
- 恢复点功能：利用 RESUME_POINTS 可以从任意中间阶段重新开始执行，提高开发效率。
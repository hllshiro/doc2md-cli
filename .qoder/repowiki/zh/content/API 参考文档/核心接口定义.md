...
AppContext 和 OutputContext 接口为整个 doc2xml-cli 工具提供了清晰的数据流架构。通过严格的类型定义和明确的职责分离，系统实现了高度的可维护性和扩展性。新增的 AI 图片识别功能进一步增强了系统的智能化处理能力，通过增强的上下文管理接口实现了多阶段的数据传递和处理协调。

...
## 核心组件
- 输入缓存与持久化
  - 类型定义：InputCache，当前包含 docxInputPath 字段，用于记录上次输入的 .docx 绝对路径。
...
章节来源
- [src/utils.ts:17-22](file://src/utils.ts#L17-L22)
- [src/utils.ts:24-35](file://src/utils.ts#L24-L35)
- [src/utils.ts:37-49](file://src/utils.ts#L37-L49)
...
participant Task as "任务如 docxInput"
...
- [src/utils.ts:24-49](file://src/utils.ts#L24-L49)
...
- 设计要点
  ...
  - 数据模型：InputCache 为可选字段对象，当前仅包含 docxInputPath。
...
- 缓存读取失败
  ...
  - 参考路径：[src/utils.ts:24-35](file://src/utils.ts#L24-L35)
- 缓存写入失败
  - 现象：输入后下次启动仍无默认值。
  - 排查：检查缓存目录权限、磁盘空间；写入失败会被静默忽略，必要时手动重建缓存文件。
  - 参考路径：[src/utils.ts:37-49](file://src/utils.ts#L37-L49)
- 输入路径校验失败
...
## 结论
工具函数模块以极简设计实现了可靠的用户输入缓存与终端交互增强，具备良好的可扩展性与健壮性。通过在任务层统一使用缓存接口，既提升了用户体验，又降低了重复输入成本。未来可在缓存持久化策略、错误可观测性与并发安全方面进一步优化。
...
- 函数：loadCache
  ...
  - 示例路径：[src/utils.ts:24-35](file://src/utils.ts#L24-L35)
...
- 函数：saveCache
  ...
  - 返回：Promise<void>。
  - 示例路径：[src/utils.ts:37-49](file://src/utils.ts#L37-L49)
  - 使用示例（任务层）：[src/tasks/docxInput.ts:41](file://src/tasks/docxInput.ts#L41)

- 类型：InputCache
  - 字段：docxInputPath（可选字符串）。
  - 示例路径：[src/utils.ts:20-22](file://src/utils.ts#L20-L22)

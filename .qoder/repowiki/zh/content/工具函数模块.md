...
- [src/tasks/imageRecognition.ts:1-616](file://src/tasks/imageRecognition.ts#L1-L616)
...
**更新** 缓存类型现已扩展支持AI相关的配置字段，包括基础URL、API密钥、模型ID和验证开关等。其中 `aiEnableValidation` 字段专门用于持久化AI图片识别的验证设置，允许用户在任务之间保持AI验证偏好的一致性。
...
- [src/tasks/imageRecognition.ts:427-432](file://src/tasks/imageRecognition.ts#L427-L432)
...
- 使用示例（路径）
  - 读取缓存配置：[src/tasks/imageRecognition.ts:376](file://src/tasks/imageRecognition.ts#L376)
  - 设置默认验证开关：[src/tasks/imageRecognition.ts:429](file://src/tasks/imageRecognition.ts#L429)
  - 写回完整配置：[src/tasks/imageRecognition.ts:432](file://src/tasks/imageRecognition.ts#L432)
  - 条件执行验证：[src/tasks/imageRecognition.ts:516-531](file://src/tasks/imageRecognition.ts#L516-L531)
...
**图表来源**
- [src/tasks/imageRecognition.ts:376-432](file://src/tasks/imageRecognition.ts#L376-L432)
- [src/tasks/imageRecognition.ts:516-531](file://src/tasks/imageRecognition.ts#L516-L531)
- [src/utils.ts:28-53](file://src/utils.ts#L28-L53)
...
**更新** 新增的 `aiEnableValidation` 字段为用户提供了对AI识别准确性的精细控制。该字段的引入使得用户可以在准确性和性能之间做出权衡：禁用验证以获得更快的处理速度，或启用验证以获得更高的识别准确性。
...
**更新** 新增的AI验证功能对性能有显著影响。启用 `aiEnableValidation` 会增加处理时间，因为每次识别都需要进行二次验证，最多进行3次重试。用户应该根据文档的复杂程度和准确性要求来权衡性能和准确性。
...
- AI配置加载失败
  ...
  - 参考路径：[src/tasks/imageRecognition.ts:376](file://src/tasks/imageRecognition.ts#L376)
- AI验证设置异常
  - 现象：aiEnableValidation 字段未正确保存或读取。
  - 排查：确认任务中正确调用了 saveCache({ aiEnableValidation })；检查缓存文件中的布尔值格式。
  - 参考路径：[src/tasks/imageRecognition.ts:432](file://src/tasks/imageRecognition.ts#L432)
- Pandoc 环境缺失
...
**更新** 新增的AI验证设置故障排查：
- AI验证设置异常：检查缓存文件中的 `aiEnableValidation` 字段是否为正确的布尔值；确认任务中正确处理了验证开关的状态变化。

**章节来源**
- [src/utils.ts:28-53](file://src/utils.ts#L28-L53)
- [src/tasks/docxInput.ts:14-26](file://src/tasks/docxInput.ts#L14-L26)
- [src/tasks/imageRecognition.ts:376](file://src/tasks/imageRecognition.ts#L376)
...
**更新** 新增的AI验证设置功能为用户提供了对AI识别准确性的精细控制。通过 `aiEnableValidation` 字段，用户可以在处理速度和识别准确性之间做出平衡：禁用验证以获得更快的处理速度，或启用验证以获得更高的识别准确性。这一功能的引入使得工具能够适应不同用户的需求和使用场景。
...
- 任务：imageRecognitionTask
  ...
  - 示例路径：[src/tasks/imageRecognition.ts:376-432](file://src/tasks/imageRecognition.ts#L376-L432)
  - 验证控制：[src/tasks/imageRecognition.ts:516-531](file://src/tasks/imageRecognition.ts#L516-L531)
...
**更新** 新增的AI验证设置最佳实践：
- 性能权衡：根据文档复杂度选择合适的验证策略；复杂数学公式文档建议启用验证，简单文本文档可禁用验证以提升性能。
- 用户教育：在AI配置界面明确告知验证功能对处理时间的影响，帮助用户做出明智选择。
- 错误处理：为AI验证过程添加适当的错误处理和降级策略，确保即使验证失败也不会完全阻止文档处理。